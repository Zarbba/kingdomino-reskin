...
function rotateTile() {
	...
	if (gameState.verticalTile === false) {
		...
		claimedTiles[claimedTiles.findIndex(findOwner)].leftMap =
			claimedTiles[claimedTiles.findIndex(findOwner)].rightMap
		claimedTiles[claimedTiles.findIndex(findOwner)].leftScore =
			claimedTiles[claimedTiles.findIndex(findOwner)].rightScore
...
function calculateScoringZones(board, x, y, type) {
	if (checkForValidBoardPosition(x, y) === false) {
...
function checkForValidBoardPosition(x, y) {
...
function resetGameState() {
	...
	gameState.winner = [0]
	gameState.round = 1
	;((gameState.isPenultimateRound = false), (gameState.isEndGame = false))
...
function validateUp(sqr) {
	if (gameState.verticalTile === false) {
		if (
			players[gameState.currentPlayer].board[sqr - 5][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].rightMap ||
			players[gameState.currentPlayer].board[sqr - 6][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].leftMap ||
		...
		if (
			players[gameState.currentPlayer].board[sqr - 10][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].leftMap ||
...
function validateDown(sqr) {
	if (gameState.verticalTile === false) {
		if (
			players[gameState.currentPlayer].board[sqr + 5][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].rightMap ||
			players[gameState.currentPlayer].board[sqr + 4][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].leftMap ||
			players[gameState.currentPlayer].board[sqr + 5][0] === `h` ||
...
function validateLeft(sqr) {
	if (gameState.verticalTile === false) {
		if (
			players[gameState.currentPlayer].board[sqr - 2][0] ===
				claimedTiles[claimedTiles.findIndex(findOwner)].leftMap ||
...
function checkValidPlacement(sqr) {
	if (
		...
		players[gameState.currentPlayer].board[
			sqr - gameState.tilePlacementModifier
		][0] !== ``
	) {
		return false
...
function placeTile(tile, id, mod) {
	players[gameState.currentPlayer].board[tile] = [
		claimedTiles[id].rightMap,
		claimedTiles[id].rightScore,
	]
	players[gameState.currentPlayer].board[tile - mod] = [
		claimedTiles[id].leftMap,
		claimedTiles[id].leftScore,
	]
	if (gameState.isEndGame === false) {
		message = `Please choose a tile to claim.`
		gameState.phase = `selection`
...
function handleClick(e) {
	...
	if (e.target.classList.contains(`reset`)) {
		...
		gameState.phase === `placement` &&
		e.target.classList.contains(`discard`)
	) {
		discardTile()
	} else if (
		gameState.phase === `selection` &&
		e.target.classList.contains(`ts-sqr`)
	) {
		if (
			availableTiles[Number(e.target.parentNode.id) - boardCoef][`owner`] ===
			undefined
		) {
			claimTile(
				players[gameState.currentPlayer],
				Number(e.target.parentNode.id) - boardCoef,
			)
		} else {
		...
		gameState.phase === `placement` &&
		e.target.classList.contains(`sqr`)
	) {
		if (checkValidPlacement(Number(e.target.id)) === true) {
			placeTile(
				Number(e.target.id),
				claimedTiles.findIndex(findOwner),
				gameState.tilePlacementModifier,
			)
		} else {
